...
function bb_verify_email( $email, $check_domain = false ) {
	...
	//if ($r != false)
	//  if (bb_check_stop_forum_spam($email))
	//   $r = false;
	return apply_filters( 'bb_verify_email', $r, $email );
}

function bb_check_stop_forum_spam($email, $ip) {
  $xml = file("http://us.stopforumspam.org/api?email=".$email."&ip=".$ip);
  foreach ($xml as $line) {
    if (strstr($line, "<appears>yes</appears>") != false)
      return true;
  }
  // save stopforumspam response to file for queries that do not appear
  $fh = fopen('stopforumspam.log', 'a');
  fwrite($fh, "Email: ".$email." IP: ".$ip."\n");
  foreach ($xml as $line)
    fwrite($fh, $line);
  fwrite($fh, "\n");
  fclose($fh);
